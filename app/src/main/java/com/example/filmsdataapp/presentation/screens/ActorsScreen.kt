...
@Composable
fun ActorsScreen(navigateToMainScreen : () -> Unit, navigateToProfilePage : () ->Unit, onMenuClick : () -> Unit, navigateToTitleScreen: (Title) -> Unit){
    Column(modifier = Modifier.fillMaxSize()
        .background(color= BackGroundColor)){
        Header(navigateToMainScreen, navigateToProfilePage, onMenuClick)
        Content("Actors", navigateToTitleScreen)
    }
}